...
export default function GameCard({ game, contentLocale }: GameCardProps) {
  ...
  const localizedCategory = game.category 
    ? (activeLocale === 'ru' ? (game.category.name_ru || game.category.name) : (game.category.name_en || game.category.name))
    : null;
    ...
    <Link href={`/games/${game.id}`} className="group relative block h-full">
      <div className="relative h-[400px] w-full overflow-hidden rounded-xl border border-white/10 bg-neutral-900 transition-all duration-300 group-hover:border-accent/50 group-hover:shadow-[0_0_30px_rgba(235,188,80,0.15)]">
        ...
        <div className="absolute inset-x-0 bottom-0 p-6">
          {localizedCategory && (
            <span className="mb-2 inline-block text-[10px] font-bold uppercase tracking-widest text-accent">
              {localizedCategory}
            </span>
          )}
